...
function getWeekNumberByDate(date) {
  ...
  const yearStart = new Date(Date.UTC(tempDate.getFullYear(), 0, 4));
  const dayOfYear =
    Math.floor(
      (tempDate - new Date(Date.UTC(tempDate.getFullYear(), 0, 1))) /
        (1000 * 60 * 60 * 24)
    ) + 1;

  return Math.ceil((dayOfYear + (yearStart.getUTCDay() || 7) - 4) / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  while (year < 9999) {
    nextDate = new Date(year, month, 13);
    if (nextDate.getDay() === 5 && nextDate >= date) {
      return nextDate;
    }
    month += 1;
    if (month > 11) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      schedule.push(formatTime(new Date(currentDate)));
